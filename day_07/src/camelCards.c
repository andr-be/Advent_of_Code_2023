...
void add_hand_p2(Collection *pile, char *line)
{
    Hand hand = {0};
    int card_no = 0;
...
void assign_type_p2(Hand *hand)
{
    ...
    case 3:
        switch (pairs)
        {
        case 0:  hand->HandType = THREE_OF_A_KIND; 
                 break;
        default: hand->HandType = FULL_HOUSE;      
                 break;
        }
        break;
    ...
    case 2:
        switch (pairs)
        {
        case 2:  hand->HandType = TWO_PAIR; 
                 break;
        default: hand->HandType = ONE_PAIR; 
                 break;
        }
        break;
...
int compare_hands_p2(const void *a, const void *b)
{
    Hand aH = *(const Hand*)a,
         bH = *(const Hand*)b;
...
{
    qsort(pile->Hands, (size_t) pile->Count, sizeof(Hand), compare_hands_p2);
}